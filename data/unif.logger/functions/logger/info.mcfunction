...
data modify storage unif.logger:cache Logger.Cache.LevelInt set value 3
...
# 打印
execute as @a[tag=unif.debug] if score $level unif.logger matches ..3 run function unif.logger:private/logger/printer with storage unif.logger:info_logs Logs[-1]